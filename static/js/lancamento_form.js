...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (djangoData.editando) {
      if (
        typeof djangoData.clientes === "string" &&
        document.getElementById("id_clientes")
      ) {
        document.getElementById("id_clientes").value = djangoData.clientes;
      }
      if (
        typeof djangoData.hospedagem === "string" &&
        document.getElementById("id_hospedagem")
      ) {
    ...
    if (
      djangoData.editando &&
      djangoData.lancamentosData &&
      djangoData.lancamentosData.length > 0
    ) {
      carregarLancamentosParaEdicao(
        djangoData.lancamentosData,
        djangoData.roteiro,
      );
    }
  ...
  function inicializarEventos() {
    ...
    document
      ...
      .addEventListener("change", aoMudarCategoria);
    document
      .getElementById("servico")
      .addEventListener("change", aoMudarServico);

    // Eventos de quantidade
    document
      .getElementById("qtdInfantil")
      .addEventListener("input", aoMudarQtdInfantil);
    document
      .getElementById("qtdMeia")
      .addEventListener("input", aoMudarQtdMeia);
    ...
    if (btnAddTransfer)
      btnAddTransfer.addEventListener("click", adicionarTransferOpcao);
    if (btnSalvarTransfer)
      btnSalvarTransfer.addEventListener("click", confirmarEdicaoTransfer);
    ...
    if (btnCopiarRoteiro) {
      console.log(
        "Botão copiar roteiro encontrado, adicionando event listener",
      );
    ...
    if (btnVerRegras)
      btnVerRegras.addEventListener("click", exibirRegrasNovamente);
  }
  ...
  function aoMudarServico() {
    ...
    fetch(url)
      ...
      .then(function (data) {
        ...
        console.log(
          "idade_isencao_min:",
          data.idade_isencao_min,
          "tipo:",
          typeof data.idade_isencao_min,
        );
        console.log(
          "idade_isencao_max:",
          data.idade_isencao_max,
          "tipo:",
          typeof data.idade_isencao_max,
        );
  ...
  function validarIdade(event) {
    ...
    if (
      servicoAtualInfo.tem_idade_minima &&
      idade < servicoAtualInfo.idade_minima
    ) {
      input.classList.add("invalido");
      input.title =
        "Idade abaixo da mínima permitida (" +
        servicoAtualInfo.idade_minima +
        " anos)";
      return;
    }
    ...
    if (
      ...
      input.title =
        "Paga Infantil (R$ " +
        parseFloat(servicoAtualInfo.valor_infantil).toFixed(2) +
        ")";
      return;
    }
    ...
    input.title =
      "Paga Inteira (R$ " +
      parseFloat(servicoAtualInfo.valor_inteira).toFixed(2) +
      ")";
  }
  ...
  function adicionarTransferOpcao(dadosPreenchimento) {
    ...
    wrapper.className =
      "transfer-fields d-flex flex-column flex-md-row align-items-stretch align-items-md-center gap-2";
    ...
    if (dados.transfer_id) {
      ...
      if (
        dados.data_transfer !== undefined &&
        dados.data_transfer !== null &&
        dados.data_transfer !== ""
      ) {
    ...
    function atualizarTransferAvulsoAoRoteiro() {
      ...
      let itensBase = servicosAdicionados.filter(function (s) {
        return (
          !s.__transfer_avulso ||
          (transferEditandoId && s.id !== transferEditandoId)
        );
      });

      // Coletar todos transfers do DOM
      const transferOpcoes = document.querySelectorAll(
        "#containerTransfers .transfer-opcao",
      );
      transferOpcoes.forEach(function (opcao, idx) {
        ...
        if (select && select.value && valorInput) {
          ...
          itensBase.push({
            id:
              transferEditandoId || "transfer_avulso_" + idx + "_" + Date.now(),
            data: dataTransfer,
    ...
    function atualizarTransferEdicaoUI() {
      ...
      if (transferEditandoId) {
        ...
        btnSalvarTransfer.innerHTML =
          '<i class="bi bi-check-circle me-2"></i>Confirmar Alteração do Transfer';
      } else {
  ...
  function adicionarServicoALista() {
    ...
    const servicoNome =
      document.getElementById("servico").selectedOptions[0].textContent;
    ...
    // Coletar quantidades
    const qtdInteira =
      parseInt(document.getElementById("qtdInteira").value) || 0;
    const qtdMeia = parseInt(document.getElementById("qtdMeia").value) || 0;
    const qtdInfantil =
      parseInt(document.getElementById("qtdInfantil").value) || 0;
    ...
    let idades = [];
    if (qtdInfantil > 0) {
      const idadesInputs = document.querySelectorAll(
        "#containerIdades input.idade-input",
      );
    ...
    let tiposMeia = [];
    if (qtdMeia > 0) {
      const tiposSelects = document.querySelectorAll(
        "#containerTiposMeia select.tipo-meia-select",
      );
    ...
    transferOpcoes.forEach(function (opcao, idx) {
      ...
      if (select && select.value && valorInput) {
        ...
        console.log(
          "  ❌ Transfer não adicionado (falta select, valor ou select vazio)",
        );
      }
    });
  ...
  function editarServico(id) {
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        if (
          servico.qtd_infantil > 0 &&
          servico.idades &&
          servico.idades.length > 0
        ) {
        ...
        if (
          ...
          gerarCamposTiposMeia(servico.qtd_meia, function () {
            ...
            tiposSelects.forEach(function (select, idx) {
              ...
              if (tipoMeiaSalvo) {
                const valorParaSelecionar =
                  tipoMeiaSalvo.id || tipoMeiaSalvo.tipo || tipoMeiaSalvo.nome;
                console.log(
                  "Definindo select",
                  idx,
                  "para:",
                  ...
                  const optionMatch = Array.from(select.options).find(
                    function (option) {
                      return (
                        ...
                        String(tipoMeiaSalvo.tipo).trim()
                      );
                    },
                  );
                  if (optionMatch) {
  ...
  function atualizarListaServicos() {
    ...
    const transfersAvulsos = servicosAdicionados.filter(function (s) {
      return s.__transfer_avulso;
    });
    ...
    servicosNormais.forEach(function (servico) {
      ...
      const idParam =
        typeof servico.id === "number" || !isNaN(Number(servico.id))
          ? servico.id
          : `'${servico.id}'`;
      let html =
        '<div class="d-flex justify-content-between align-items-start mb-2">' +
        "<div>" +
        '<h6 class="mb-1"><i class="bi bi-calendar-event me-1"></i>' +
        ...
        "</div>" +
        '<div class="btn-group">' +
        '<button class="btn btn-sm btn-outline-primary" onclick="window.editarServico(' +
        idParam +
        ')">' +
        '<i class="bi bi-pencil"></i>' +
        "</button>" +
        '<button class="btn btn-sm btn-outline-danger" onclick="window.removerServico(' +
        idParam +
        ')">' +
    ...
    transfersAvulsos.forEach(function (transfer) {
      ...
      div.className = "servico-adicionado";
      const idParam =
        typeof transfer.id === "number" || !isNaN(Number(transfer.id))
          ? transfer.id
          : `'${transfer.id}'`;
      let html =
        '<div class="d-flex justify-content-between align-items-start mb-2">' +
        "<div>" +
        '<h6 class="mb-1"><i class="bi bi-calendar-event me-1"></i>' +
        formatarData(transfer.data) +
        "</h6>" +
        '<p class="mb-1"><strong>' +
        transfer.servico_nome +
        "</strong></p>" +
        "</div>" +
        '<div class="btn-group">' +
        '<button class="btn btn-sm btn-outline-primary" onclick="window.editarTransfer(' +
        idParam +
        ')">' +
        '<i class="bi bi-pencil"></i>' +
        "</button>" +
      ...
      if (transfer.transfers && transfer.transfers.length > 0) {
        transfer.transfers.forEach(function (t) {
          const nomeExibicao =
            t.nome_personalizado || t.nome_exibicao || t.nome;
          html +=
            '<span class="badge bg-warning text-dark me-1">Transfer: ' +
            nomeExibicao +
            " - R$ " +
  ...
  function atualizarResumoTotais() {
    ...
    const servicosReais = servicosAdicionados.filter(
      (s) => !s.__transfer_avulso,
    );
    const transfersAvulsos = servicosAdicionados.filter(
      (s) => s.__transfer_avulso,
    );
    if (servicosReais.length === 0 && transfersAvulsos.length === 0) {
      container.innerHTML =
        '<div class="text-center text-muted py-3"><small>Nenhum serviço ou transfer adicionado</small></div>';
    ...
    servicosReais.forEach(function (servico) {
      ...
      html +=
        '<strong class="d-block">' + idx++ + ". " + nomeServico + "</strong>";
      html +=
      ...
      html +=
        ...
        (qtdInteira + qtdMeia + qtdInfantil) +
        " PAX</span>";
      html += "</div>";
      ...
      if (qtdInteira > 0) {
        ...
        html +=
          ...
          qtdInteira +
          " Inteira(s) × R$ " +
          valorInteira.toFixed(2).replace(".", ",") +
          "</span>";
        html +=
          '<span class="text-primary">R$ ' +
          subtotalInteira.toFixed(2).replace(".", ",") +
          "</span>";
        html += "</div>";
      }
      if (qtdMeia > 0) {
        ...
        html +=
          ...
          qtdMeia +
          " Meia(s) × R$ " +
          valorMeia.toFixed(2).replace(".", ",") +
          "</span>";
        html +=
          '<span class="text-info">R$ ' +
          subtotalMeia.toFixed(2).replace(".", ",") +
          "</span>";
        html += "</div>";
      }
      if (qtdInfantil > 0) {
        ...
        html +=
          ...
          qtdInfantil +
          " Infantil(is) × R$ " +
          valorInfantil.toFixed(2).replace(".", ",") +
          "</span>";
        html +=
          '<span class="text-success">R$ ' +
          subtotalInfantil.toFixed(2).replace(".", ",") +
          "</span>";
        html += "</div>";
      }
      html += '<div class="d-flex justify-content-end mt-2 pt-2 border-top">';
      html +=
        '<strong>Subtotal: <span class="text-dark">R$ ' +
        subtotalServico.toFixed(2).replace(".", ",") +
        "</span></strong>";
      html += "</div>";
      html += "</div>";
      html += "</div>";
    ...
    transfersAvulsos.forEach(function (transfer) {
      ...
      html +=
        '<small class="text-muted">' +
        formatarDataBrasileira(transfer.data) +
        "</small>";
      html += "</div>";
  ...
  window.editarTransfer = function (id) {
    ...
    const transfer = servicosAdicionados.find(function (s) {
      return s.id === id;
    });
    if (!transfer) return;
    ...
    if (transferItens.length === 0) {
      ...
      transferItens.forEach(function (item) {
        adicionarTransferOpcao({
          ...
          nome_personalizado:
            item.nome_personalizado || item.nome_exibicao || item.nome || "",
          valor: item.valor,
  ...
  function copiarRoteiro() {
    ...
    try {
      ...
      alert(
        "Erro ao copiar. Por favor, selecione e copie manualmente (CTRL+C).",
      );
    }
  }
  ...
  function salvarOrdemServico() {
    ...
    const transferSemData = servicosAdicionados.find(function (item) {
      return (
        item.__transfer_avulso &&
        (!item.data_transfer || !String(item.data_transfer).trim())
      );
    });
    if (transferSemData) {
    ...
    console.log(
      "Dados completos:",
      JSON.stringify(servicosAdicionados, null, 2),
    );

    const dados = {
      servicos: servicosAdicionados,
      roteiro: document.getElementById("roteiroPreview").textContent,
      clientes: document.getElementById("id_clientes")
        ? document.getElementById("id_clientes").value
        : "",
      hospagem: document.getElementById("id_hospedagem")
        ? document.getElementById("id_hospedagem").value
        : "",
    };

    // URL varia se está editando ou criando
    let url =
      djangoData.urls.ordemServicoCreate || djangoData.urls.lancamentoCreate;
      ...
      .then(function (response) {
        if (response.ok) {
          ...
          window.location.href =
            djangoData.urls.ordemServicoList || djangoData.urls.lancamentoList;
        } else {
    ...
    if (lancamentos && lancamentos.length > 0) {
      ...
      lancamentos.forEach(function (lancamento, index) {
        ...
        const servicoCompleto = {
          ...
          valor_transfer_volta:
            parseFloat(lancamento.valor_transfer_volta) || 0,
          descricao: lancamento.descricao || "",
          info: lancamento.info || {},
        };
        // Se for transfer avulso, manter flags especiais
        if (lancamento.__transfer_avulso)
          servicoCompleto.__transfer_avulso = true;
        if (lancamento.__nao_exibir_card)
          servicoCompleto.__nao_exibir_card = true;
      ...
      atualizarListaServicos();
      atualizarResumoTotais();
      atualizarRoteiro(); // Monta o roteiro novo dinamicamente

      // REMOVIDO: O bloco que injetava a variável "roteiro" (antiga) do banco de dados
      // para forçar a exibição do novo layout imediatamente.
  ...
  function atualizarRegrasServico(info) {
    ...
    if (info.possui_isencao) {
      ...
      li.className = "mb-2";
      const idadeMin =
        info.idade_isencao_min !== undefined ? info.idade_isencao_min : "?";
      const idadeMax =
        info.idade_isencao_max !== undefined ? info.idade_isencao_max : "?";
      li.innerHTML =
        ...
        " a " +
        idadeMax +
        " anos são <strong>GRATUITAS</strong>" +
    ...
    if (info.permite_infantil) {
      ...
      li.innerHTML =
        ...
        info.idade_minima_infantil +
        " a " +
        info.idade_maxima_infantil +
        " anos" +
        '<br><small class="text-muted ms-4">Valor: <strong>R$ ' +
        parseFloat(info.valor_infantil).toFixed(2) +
        "</strong> por criança</small>";
      listaRegras.appendChild(li);
      temRegras = true;
    }
    ...
    if (info.tem_idade_minima) {
      ...
      li.innerHTML =
        ...
        '<strong class="text-danger">IDADE MÍNIMA:</strong> ' +
        info.idade_minima +
        " anos" +
    ...
    if (info.aceita_meia_entrada) {
      ...
      li.innerHTML =
        ...
        '<br><small class="text-muted ms-4">Valor: <strong>R$ ' +
        parseFloat(info.valor_meia).toFixed(2) +
        "</strong> por pessoa</small>" +
        regrasTexto;
      listaRegras.appendChild(li);
      temRegras = true;
    } else {
    ...
    li.innerHTML =
      ...
      '<strong class="text-primary">VALOR INTEIRA:</strong> R$ ' +
      parseFloat(info.valor_inteira).toFixed(2) +
  ...
  function exibirRegrasNovamente() {
    ...
    if (regrasDiv && regrasDiv.style.display !== "none") {
      ...
      alert(
        "⚠️ Por favor, selecione um serviço primeiro para ver as regras de negócio.",
      );
    }
  }
